...
class Crawl:
    ...
    def __init__(self, port=5001, username="", password="", start_row=0, row_count=50000, output_dir="cnipa"):
        self.port = port
    ...
    "sec-ch-ua-platform": "\"Windows\""
        ...
        # Create output directories for this account
    ...
    def login(self,browser):
        try:
            ...
            browser.ele('xpath://input[@placeholder="手机号/证件号码"]').input(self.username)
            browser.ele('xpath://input[@placeholder="请输入密码"]').input(self.password)
    ...
    def main(self):
        # Read records with specified range
        end_row = self.start_row + self.row_count
        records = pd.read_csv("./after_2010.csv",encoding="utf-8").to_dict(orient="records")[self.start_row:end_row]
        
        # Use specified port for browser
        browser = ChromiumPage(self.port)
        ...
        for record_index,record in enumerate(records):
            ...
            if os.path.exists(f"./{self.output_dir}/pdf/CN{keyword_no}.pdf") or os.path.exists(f"./{self.output_dir}/error_pdf/CN{keyword_no}.pdf"):
                continue
            # browser.get("https://cpquery.cponline.cnipa.gov.cn/chinesepatent/index")
            ...
            for _record in response["data"]["records"]:
                ...
                if _response["data"]["wenjianhzm"] == "pdf":
                    for index,item in enumerate(_response["data"]["ossLujingList"]):
                        ...
                        while True:
                            try:
                                ...
                                pass
                        with open(f"./{self.output_dir}/pdf/CN{keyword_no}.pdf","wb") as fp:
                            fp.write(content)  
                    ...
                    if img_error_count >= 5:
                        ...
                        error_fp.flush()
                        continue
                    output_pdf = f"./{self.output_dir}/pdf/CN{keyword_no}.pdf"  # 输出的 PDF 文件名
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CNIPA Crawler with multiple accounts')
    parser.add_argument('--port', type=int, default=5001, help='Browser port')
    parser.add_argument('--username', type=str, default="", help='Username')
    parser.add_argument('--password', type=str, default="", help='Password')
    parser.add_argument('--start_row', type=int, default=0, help='Start row in CSV file')
    parser.add_argument('--row_count', type=int, default=50000, help='Number of rows to process')
    parser.add_argument('--output_dir', type=str, default="cnipa", help='Output directory name')
    
    args = parser.parse_args()
    
    # Initialize and run crawler with provided parameters
    crawler = Crawl(
        port=args.port,
